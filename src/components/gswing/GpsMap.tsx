...
import { synthesizePremiumHoleWithOsm, enrichMappedHoleWithAuto } from "@/lib/gswing-auto-premium";
    ...
    (async () => {
      ...
      if (!courseMap) {
          ...
          if (auto) {
            ...
            console.info("[gswing.gps] premium geometry source", {
              buildSha: __GSWING_BUILD_SHA__,
              course: selectedCourse.name,
              hole,
              source: "auto-osm-or-generated",
              reason: "no-course-map",
              mapped_hole_id: auto.id,
              autoGenerated: true,
              fairwayPolygonPts: auto.fairwayPolygon?.length ?? 0,
              greenPolygonPts: auto.green.polygon?.length ?? 0,
              teePolygonPts: auto.teePolygon?.length ?? 0,
              holeBoundaryPts: auto.holeBoundary?.length ?? 0,
              hazardPolygons: auto.hazards.filter((h) => h.polygon?.length).length,
            });
          } else {
            setMappedHole(null);
        ...
        if (cancelled) return;
        if (hp) {
          // Backfill any premium visual layers the DB mapping is missing
          // (fairway/green/tee/boundary polygons, hazard shapes) using
          // bundled + OSM synthesis so every hole renders at Hole 6 fidelity.
          const needsEnrichment =
            !hp.fairwayPolygon ||
            !hp.green.polygon ||
            !hp.teePolygon ||
            !hp.holeBoundary ||
            !hp.hazards.some((h) => h.polygon);
          let enriched: MappedHole = hp;
          if (needsEnrichment) {
            const auto = await synthesizePremiumHoleWithOsm(gps, hole, selectedCenter).catch(() => null);
            if (cancelled) return;
            if (auto) enriched = enrichMappedHoleWithAuto(hp, auto);
          }
          setMappedHole(enriched);
          setMappedIsAuto(false);
          setMappingStatus("mapped");
          console.info("[gswing.gps] premium geometry source", {
            buildSha: __GSWING_BUILD_SHA__,
            ...
            hole,
            source: needsEnrichment ? "mapped-feature-polygons+auto-fill" : "mapped-feature-polygons",
            reason: "course-map-hole-found",
            course_map_id: courseMap.id,
            mapped_hole_id: hp.id,
            autoGenerated: false,
            fairwayPolygonPts: enriched.fairwayPolygon?.length ?? 0,
            greenPolygonPts: enriched.green.polygon?.length ?? 0,
            teePolygonPts: enriched.teePolygon?.length ?? 0,
            holeBoundaryPts: enriched.holeBoundary?.length ?? 0,
            hazardPolygons: enriched.hazards.filter((h) => h.polygon?.length).length,
          });
        } else {
          ...
          if (auto) {
            ...
            console.info("[gswing.gps] premium geometry source", {
              ...
              hole,
              source: "auto-osm-or-generated",
              reason: "mapped-course-missing-hole",
              course_map_id: courseMap.id,
              mapped_hole_id: auto.id,
              autoGenerated: true,
              fairwayPolygonPts: auto.fairwayPolygon?.length ?? 0,
