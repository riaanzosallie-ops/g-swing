      ...
      if (!courseMap) {
        ...
        if (hp) {
          ...
          console.info("[gswing.gps] premium geometry source", {
            ...
            hole,
            source: "mapped-feature-polygons",
            reason: "course-map-hole-found",
            course_map_id: courseMap.id,
            mapped_hole_id: hp.id,
            autoGenerated: false,
            fairwayPolygonPts: hp.fairwayPolygon?.length ?? 0,
            greenPolygonPts: hp.green.polygon?.length ?? 0,
            teePolygonPts: hp.teePolygon?.length ?? 0,
            holeBoundaryPts: hp.holeBoundary?.length ?? 0,
            hazardPolygons: hp.hazards.filter((h) => h.polygon?.length).length,
          });
        } else {
